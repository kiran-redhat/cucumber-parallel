...
module.exports = function tasks() {
    ...
    function getFeatures(fromDirectory, parallelRunType) {
        ...
        if (fromDirectory.indexOf('.feature')!== -1) return path.join(process.cwd(),fromDirectory);
    ...
    function getScenarios(fromDirectory, parallelRunType) {
        ...
        var getScenariosForFeature = function (featureFilePath) {
        ...
        if (fromDirectory.indexOf(':')!== -1){
            return path.join(process.cwd(),fromDirectory);
        }else if (fromDirectory.indexOf('.feature')!== -1){
            getScenariosForFeature(path.join(process.cwd(),fromDirectory));
        }else{
            getFeatures(fromDirectory).forEach(function (featureFilePath) {
                getScenariosForFeature(featureFilePath);
            });
        }
    ...
    function getTasks(featuresOrDirectoryPath, cucumberProgram) {
        ...
        //Changes to allow multiple feature files/paths to be run in parallel ------------------------------

        //Loop through the features/paths list and get all the features from each of the directories
        let allFeatures = featuresOrDirectoryPath.map(function(featurePath){
            var fromDirectory = featurePath;
        ...
        allFeatures = [].concat.apply([], allFeatures);
        var uniqueArray = (arrArg) => arrArg.filter((elem, pos, arr) => arr.indexOf(elem) == pos);
        allFeatures = uniqueArray(allFeatures);
